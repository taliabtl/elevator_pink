...
namespace elevator_simulation.ViewModels
{
    public class MainViewModel : ViewModelBase
    {
        ...
        // LOOK algoritması için son yön hafızası
        private int _lastDirection1 = 0;
        private int _lastDirection2 = 0;

        // ML park seyahati devam ediyor mu (skor hesabında Idle gibi değerlendirmek için)
        private bool _elevator1IsParking = false;
        ...
        private async Task ParkElevatorAtFloor(int targetFloor)
        {
            ...
            {
                // Yeni çağrı veya istek gelirse anında iptal et
        ...
        /// </summary>
        private int CalculateElevatorScore(int callingFloor, int elevatorFloor, Models.ElevatorState elevatorState, int distance, bool isBusy, bool isParking = false)
        {
            int score = 100;

            // Meşgul cezası (park yolculuğu meşgul sayılmaz)
            if (isBusy && !isParking) score -= 30;

            // Mesafe cezası — park halindeyken mesafe daha az cezalandırılır
            // çünkü park yolculuğu kesilip bu çağrıya yönelebilir
            score -= isParking ? distance * 2 : distance * 3;

            int elevatorToCall = callingFloor - elevatorFloor;

            if (isParking)
            {
                // Park seyahati sırasında gelen çağrı: asansör yönünü değiştirebilir,
                // ama mevcut konumu gerçek konumu — Idle bonusu ver
                score += 30;
            }
            else if (elevatorState == Models.ElevatorState.MovingUp && elevatorToCall > 0)
            {
                score += 40; // aynı yönde, yolda alınabilir
            }
        ...
        {
            if (parameter is not int callingFloor) return;
            ...
            // ── Her iki asansörün o kata olan uzaklığı ve yük durumu ─────────
            int distance1 = Math.Abs(_currentFloor  - callingFloor);
            int distance2 = Math.Abs(_currentFloor2 - callingFloor);

            bool el1Busy = _isProcessingRequests || (_elevator.State != Models.ElevatorState.Idle && !_elevator1IsParking);
            bool el2Busy = _isProcessingRequests2 || _elevator2.State != Models.ElevatorState.Idle;

            // Bekleyen istek yükü (o asansörde kaç pending var)
            int load1 = _pendingRequests.Count(r  => r.Status == RequestStatus.Pending);
            int load2 = _pendingRequests2.Count(r => r.Status == RequestStatus.Pending);

            // ── Skor hesapla ─────────────────────────────────────────────────
            int score1 = CalculateElevatorScore(callingFloor, _currentFloor,  _elevator.State,  distance1, el1Busy, _elevator1IsParking);
            int score2 = CalculateElevatorScore(callingFloor, _currentFloor2, _elevator2.State, distance2, el2Busy);

            // Yük dengeleme: daha az bekleyeni olan asansöre bonus
            if (load1 > load2) score2 += (load1 - load2) * 5;
            else if (load2 > load1) score1 += (load2 - load1) * 5;

            // Eğer her iki skor da çok yakınsa (fark ≤10) daha az yüklüye git
            if (Math.Abs(score1 - score2) <= 10)
            {
                if (load1 <= load2) score1 += 15;
                else                score2 += 15;
            }

            // ── Asansör seç ──────────────────────────────────────────────────
            int selected = score2 > score1 ? 2 : 1;

            AddStatusMessage($"[Skor] Asansör 1: {score1}(yük:{load1}), Asansör 2: {score2}(yük:{load2}) → Asansör {selected} seçildi");
        ...
        private async Task HandleElevatorCall(int callingFloor, int elevatorNumber, int currentFloor, List<PassengerRequest> pendingRequests)
        {
            ...
            {
                // Park seyahatindeki asansör de idle gibi davranabilir (çağrı keser park'ı)
                1 => _elevator.State == Models.ElevatorState.Idle || _elevator1IsParking,
            ...
            // Asansör 1 park yolculuğundayken çağrı seçildi → park'ı kes
            if (elevatorNumber == 1 && _elevator1IsParking)
            {
                _elevator1IsParking = false;
                _elevator.State = Models.ElevatorState.Idle;
                ElevatorStateDisplay = "Beklemede";
                AddStatusMessage($"📊 [Saatlik Park] Çağrı geldi, park iptal → Kat {callingFloor}'a yönleniyor.");
            }
        ...
        private async Task ProcessRequests()
        {
                   ...
                   _destinationFloors.Count > 0)
            {
                ...
                AddStatusMessage($"[LOOK-1] Yön={direction}, LastDir={_lastDirection1}, Kat={_currentFloor}, Dest=[{string.Join(",", _destinationFloors)}], Pickup=[{string.Join(",", _pendingRequests.Where(r => r.Status == RequestStatus.Pending).Select(r => r.PickupFloor))}]");
                ...
                ElevatorStateDisplay = direction > 0 ? "Yukarı Gidiyor" : "Aşağı Gidiyor";
                _lastDirection1 = direction; // yön hafızasını güncelle
        ...
        {
            // LOOK Algoritması: drop-off + pending pickup katlarını tek kümede toplar.
            // _lastDirection1 ile yön hafızası korunur (kapı açılıp kapandıktan sonra da).
            var allStops = new HashSet<int>();
            foreach (var f in _destinationFloors)
                allStops.Add(f);
            foreach (var r in _pendingRequests.Where(r => r.Status == RequestStatus.Pending))
                allStops.Add(r.PickupFloor);
            ...
            bool hasUp   = allStops.Any(f => f > _currentFloor);
            bool hasDown = allStops.Any(f => f < _currentFloor);

            // Mevcut yönde (hafızadan) durak varsa devam et
            if (_lastDirection1 == 1  && hasUp)   return  1;
            if (_lastDirection1 == -1 && hasDown) return -1;

            // Hafıza tükendi veya 0 — state'e bak
            ...
            // Tek yönde durak varsa
            if (hasUp   && !hasDown) return  1;
            if (hasDown && !hasUp)   return -1;

            // Her iki yanda da durak var → en yakın tarafa git
            int closestUp   = allStops.Where(f => f > _currentFloor).Min();
            int closestDown = allStops.Where(f => f < _currentFloor).Max();
        ...
        private async Task ProcessRequests2()
        {
                   ...
                   _destinationFloors2.Count > 0)
            {
                ...
                _lastDirection2 = direction; // yön hafızasını güncelle
        ...
        {
            // LOOK Algoritması: drop-off + pending pickup katlarını tek kümede toplar.
            // _lastDirection2 ile yön hafızası korunur (kapı açılıp kapandıktan sonra da).
            var allStops2 = new HashSet<int>();
            foreach (var f in _destinationFloors2)
                allStops2.Add(f);
            foreach (var r in _pendingRequests2.Where(r => r.Status == RequestStatus.Pending))
                allStops2.Add(r.PickupFloor);
            ...
            bool hasUp2   = allStops2.Any(f => f > _currentFloor2);
            bool hasDown2 = allStops2.Any(f => f < _currentFloor2);

            // Mevcut yönde (hafızadan) durak varsa devam et
            if (_lastDirection2 == 1  && hasUp2)   return  1;
            if (_lastDirection2 == -1 && hasDown2) return -1;

            // Hafıza tükendi veya 0 — state'e bak
            if (_elevator2.State == Models.ElevatorState.MovingUp   && hasUp2)   return  1;
            if (_elevator2.State == Models.ElevatorState.MovingDown && hasDown2) return -1;

            // Tek yönde durak varsa
            if (hasUp2   && !hasDown2) return  1;
            if (hasDown2 && !hasUp2)   return -1;

            // Her iki yanda da durak var → en yakın tarafa git
            int closestUp2   = allStops2.Where(f => f > _currentFloor2).Min();
            int closestDown2 = allStops2.Where(f => f < _currentFloor2).Max();
            return (closestUp2 - _currentFloor2) <= (_currentFloor2 - closestDown2) ? 1 : -1;
        }
        ...
        /// <summary>
        /// Dashboard için simülasyon metriklerini hesaplar.
        /// </summary>
        public Models.PerformanceMetrics GetPerformanceMetrics()
        {
            return _mlDataCollector.ComputeMetrics();
        }
    }
}
